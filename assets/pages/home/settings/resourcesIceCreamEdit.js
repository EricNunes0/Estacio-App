...
export default function ResourcesIceCreamEdit() {
    ...
    const updateItem = async (id) => {
        ...
        let i = 0;
        for(const resourceItem of resourcesObject.sorvete[resource].items) {
            if(resourceItem.id === id) {
                if(resource == "tamanho") {
                    let newSizeLabel = sizeConvert(itemValue);
                    resourcesObject.sorvete[resource].items[i].value = parseFloat(itemValue);
                    resourcesObject.sorvete[resource].items[i].label = newSizeLabel;
                } else if(resource == "calda" || resource == "sabores" || resource == "condimentos" || resource == "adicionais") {
                    resourcesObject.sorvete[resource].items[i].value = itemValue;
                    resourcesObject.sorvete[resource].items[i].label = itemValue;
                };
                resourcesObject.sorvete[resource].items[i].price = getPrice();
            } else {
                i++;
            }
        };
